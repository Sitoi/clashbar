...
final class StatusItemContentView: NSView {
    ...
    private static func makeBitmapRep(
        pointSize: NSSize,
        scale: CGFloat,
        draw: (NSGraphicsContext) -> Void) -> NSBitmapImageRep?
    {
    ...
    {
        self.makeBitmapRep(pointSize: pointSize, scale: scale) { _ in
            let paragraph = NSMutableParagraphStyle()
            paragraph.alignment = .right
            paragraph.lineBreakMode = .byTruncatingHead
            let attributes: [NSAttributedString.Key: Any] = [
                .font: NSFont.monospacedDigitSystemFont(
                    ofSize: MenuBarLayoutTokens.FontSize.statusBar,
                    weight: .semibold),
    ...
    private static func makeBrandStatusIconRepresentation(
        ...
        scale: CGFloat) -> NSBitmapImageRep?
    {
        self.makeBitmapRep(pointSize: pointSize, scale: scale) { context in
